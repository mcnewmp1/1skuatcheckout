...
function ($scope, $routeParams, $location, $451, Order, OrderConfig, User) {
	...
	$scope.checkOut = function() {
		...
		Order.checkQuantityLimitedItems($scope.currentOrder
		,function(){
			$scope.displayLoadingIndicator = true;
			if (!$scope.isEditforApproval)
				OrderConfig.address($scope.currentOrder, $scope.user);
  ...
  setTimeout(function(){ Order.checkQuantityLimitedItems($scope.currentOrder,null,null,function(msg){ alert(msg); }); },500);
